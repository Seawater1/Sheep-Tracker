...
#include "LoRaWan-Arduino.h"   // SX126x P2P radio control
#include <SPI.h>
...
// =========================
// LoRa P2P settings (MUST match receiver)
// =========================
...
void setup()
{
  ...
  Serial.println("=====================================");
  Serial.println("RAK11300 GNSS + Battery + LoRa TX");
  Serial.println("=====================================");
  ...
  Radio.SetTxConfig(MODEM_LORA, TX_OUTPUT_POWER, 0, LORA_BANDWIDTH,
                    ...
                    true, 0, 0, false, TX_TIMEOUT_VALUE);

  Serial.println("LoRa ready. Sending compact GPS+BATT every 1s...");
}
...
void loop()
{
  if (millis() - g_lastTime >= 1000)
  {
    ...
    // ONE clean print line per second (your original)
    ...
    // ===== LoRa payload with TIME + battery volts + percent =====
    if (!txBusy)
    {
      // Get GPS time (UTC)
      uint16_t year = g_myGNSS.getYear();
      uint8_t month = g_myGNSS.getMonth();
      uint8_t day   = g_myGNSS.getDay();
      uint8_t hour  = g_myGNSS.getHour();
      uint8_t min   = g_myGNSS.getMinute();
      uint8_t sec   = g_myGNSS.getSecond();

      char timestamp[13] = "000000000000";
      ...
      float batt_volts = vbat_mv / 1000.0;

      char payload[64];
      snprintf(payload, sizeof(payload),
        "%s,%.5f,%.5f,%.1f,%.1f,%.1f,%u,%.4f,%u",
        timestamp,
